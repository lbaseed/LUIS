...
function sumCart(){
	...
	if ($('#list_items_cart li').length>0){
...
$("#payment_borrow").click(function() {
	var check=$("#status").text(); //check if sold
	var status="sold"
	if(check=="borrow"){payment_borrow(status);}
	else{alert("Items already sold/returned");}
...
$("#get_receipt").click(function(){//get receipt of borrowed record
	var ID=$("#trans_id").val();
	$('#fullModal').modal({ backdrop: 'static' });
						$("#modal_full_trans_ref").html(ID);
...
function return_borrow(){
	...
	var cust_id= parseFloat($("#cust_id").val()); //from a hidden input
	var trans_id= $("#trans_id").val();//also
	var balance=parseFloat($("#balance").val());// label
	var amount_tendered = parseFloat($("#total_tendered_label").text().replace(",","")); //label
	var newDebt=balance-amount_tendered;
	
	*/
	var cust_id= parseFloat($("#cust_id").val()); //from a hidden input
	var trans_id= $("#trans_id").val();//also
	
	var data=[cust_id,trans_id];
	
	$.ajax({
					type: 'POST',
					url: "api/insert_borrow.php",
					data: {Data: data},
...
function payment_borrow(status){ //when sold and cash returned
	...
	*/
	...
	var data=[cust_id,trans_id,status];
...
function getTransactionReceipt(refID){
	...
	var ourRequest = new XMLHttpRequest();
		...
		ourRequest.onload = function(){
			...
			if (mop == "borrow" ) {mop_text = "B/borrw";}
			if (mop == "sold" ) {mop_text = "B/sold";}
			if (mop == "return" ) {mop_text = "B/returned";}
