			...
			if (qty > 0){
				...
				if (stockQty >= qty){
					...
					if ($('#list_items_cart  li').size()==0){
						...
						$("#list_items_cart").append('<li class="list-group-item" id=item_0>' + 
													name + ' <span class="badge bg-cyan" id="item_price_0">'+ price +'</span> <span class="badge bg-cyan" id="item_qty_0">'+ qty +'</span> <span class="badge bg-cyan" id="item_id_0">'+ item +'</span><input type="hidden" id="item_serial_0" value="' + serial + '" /> <a href="javascript:void(0);" id="remove">&times;</a></li>');

					}
					else{
						var item_index = $('#list_items_cart li').size();

						$("#list_items_cart").append('<li class="list-group-item" id=item_'+ item_index +'>' + 
													name + '  <span class="badge bg-cyan" id="item_price_'+ item_index +'">'+ price +'</span> <span class="badge bg-cyan" id="item_qty_'+ item_index +'">'+ qty +'</span> <span class="badge bg-cyan" id="item_id_'+ item_index +'">'+ item +'</span> <input type="hidden" id="item_serial_' + item_index + '" value="' + serial + '" />  <a href="javascript:void(0);" id="remove">&times;</a> </li>');
				...
				if ($('#list_items_cart li').size()>0){
					...
					//var vat = total * 0.05;
					//var total_with_vat =  total;
					//$("#vat_label").html(vat);
					
					//$("#amount_payable_label").html(total_with_vat);
				}
...
function sumCart(){
	...
	var size = $('#list_items_cart li').size();
	
	for (var i=0; i<size; i++){

		var ps = $("#item_price_"+i).html();
		var qt = $("#item_qty_"+i).html();

		if (ps === undefined) {alert(i); continue; }
...
function thermal_receipt(){
	...
	if (cust_name =="" && cust_address == "" && cust_phone == ""){
			...
			if ($('#list_items_cart ul li').size()>0 ) {
